...
func (suite *IBCTestingSuite) SetupTest() {
	...
	err := suite.EvmosChain.App.(*app.Blockx).BankKeeper.MintCoins(suite.EvmosChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = suite.EvmosChain.App.(*app.Blockx).BankKeeper.SendCoinsFromModuleToAccount(suite.EvmosChain.GetContext(), inflationtypes.ModuleName, suite.IBCOsmosisChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)

	// Mint coins on the osmosis side which we'll use to unlock our abcx
	...
	// Mint coins on the cosmos side which we'll use to unlock our abcx
	...
	suite.EvmosChain.App.(*app.Blockx).ClaimsKeeper.SetParams(suite.EvmosChain.GetContext(), claimparams)

	params := types.DefaultParams()
	params.EnableRecovery = true
	suite.EvmosChain.App.(*app.Blockx).RecoveryKeeper.SetParams(suite.EvmosChain.GetContext(), params)
...
var (
	...
	abcxDenomtrace = transfertypes.DenomTrace{
		Path:      "transfer/channel-0",
		BaseDenom: "abcx",
	}
	abcxIbcdenom = abcxDenomtrace.IBCDenom()
