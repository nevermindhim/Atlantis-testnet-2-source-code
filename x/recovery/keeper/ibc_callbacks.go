...
import (
	...
	"github.com/defi-ventures/bcx-testnet-7/ibc"
	blockx "github.com/defi-ventures/bcx-testnet-7/types"
...
//   - sends over all blockx native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `blockx1` and the original bech32 format
	...
	// a sender's own account on blockx (sender == recipient)
	...
	if account != nil && blockx.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the blockx chain
...
// the IBC denomination, i.e port and channel on blockx for the voucher. It
// returns an error if:
