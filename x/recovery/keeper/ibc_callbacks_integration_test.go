...
import (
	...
	claimtypes "github.com/defi-ventures/bcx-testnet-7/x/claims/types"
...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	coinEvmos := sdk.NewCoin("abcx", sdk.NewInt(10000))
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			s.EvmosChain.App.(*app.Blockx).ClaimsKeeper.SetParams(s.EvmosChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			nativeEvmos := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, "abcx")
			Expect(nativeEvmos).To(Equal(coinEvmos))
			ibcOsmo := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, uosmoIbcdenom)
		...
		Describe("to the sender's own eth_secp256k1 account on Evmos (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.EvmosChain.App.(*app.Blockx).RecoveryKeeper.SetParams(s.EvmosChain.GetContext(), params)
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.EvmosChain.App.(*app.Blockx).ClaimsKeeper.SetClaimsRecord(s.EvmosChain.GetContext(), senderAcc, claim)
					})
				...
				Context("with completed actions", func() {
					...
					BeforeEach(func() {
						amt := sdk.NewInt(int64(100))
						coins := sdk.NewCoins(sdk.NewCoin("abcx", sdk.NewInt(int64(75))))
						claim = claimtypes.NewClaimsRecord(amt)
						claim.MarkClaimed(claimtypes.ActionIBCTransfer)
						s.EvmosChain.App.(*app.Blockx).ClaimsKeeper.SetClaimsRecord(s.EvmosChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.EvmosChain.App.(*app.Blockx).BankKeeper, s.EvmosChain.GetContext(), claimtypes.ModuleName, coins)
						s.Require().NoError(err)

						// abcx & ibc tokens that originated from the sender's chain
						s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.IBCOsmosisChain, coinOsmo.Denom, coinOsmo.Amount.Int64(), sender, receiver, 1)
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), "abcx")
						Expect(balanceEscrow).To(Equal(coinEvmos))
						ibcOsmo := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, uosmoIbcdenom)
						Expect(ibcOsmo.IsZero()).To(BeTrue())

						// Relay both packets that were sent in the ibc_callback
						err := s.pathOsmosisEvmos.RelayPacket(CreatePacket("10000", "abcx", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						...
						// Check that the abcx were recovered
						nativeEvmos := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, "abcx")
						Expect(nativeEvmos.IsZero()).To(BeTrue())
						ibcEvmos := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, abcxIbcdenom)
						Expect(ibcEvmos).To(Equal(sdk.NewCoin(abcxIbcdenom, coinEvmos.Amount)))
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.EvmosChain.App.(*app.Blockx).ClaimsKeeper.GetClaimsRecord(s.EvmosChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						// abcx was recovered from user address
						nativeEvmos := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, "abcx")
						Expect(nativeEvmos.IsZero()).To(BeTrue())
						ibcEvmos := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, abcxIbcdenom)
						Expect(ibcEvmos).To(Equal(sdk.NewCoin(abcxIbcdenom, coinEvmos.Amount)))
						...
						ibcAtom := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, uatomIbcdenom)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						...
						osmoIBCAtom := s.EvmosChain.App.(*app.Blockx).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, uatomOsmoIbcdenom)
