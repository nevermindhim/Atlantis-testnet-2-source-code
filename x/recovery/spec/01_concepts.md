...
1. User attempts to recover `1000abcx` that are stuck on the Evmos chain.
...
3. Evmos receives the transaction, and goes through the IBC stack:
    ...
    3. **Recovery Middleware**: since `sender=receiver` -> recover user balance (`1000abcx` and `100uosmo`) by sending an IBC transfer from `receiver` to the `sender` on the Osmosis chain.
4. User receives `100uosmo` and `1000abcx` (IBC voucher) on Osmosis.
