...
const (
	// MainnetChainID defines the Blockx EIP155 chain ID for mainnet
	MainnetChainID = "blockx_12346"
	// TestnetChainID defines the Blockx EIP155 chain ID for testnet
	TestnetChainID = "blockx_12345"
)

// IsMainnet returns true if the chain-id has the Blockx mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Blockx testnet EIP155 chain prefix.
...
// GetBlockxAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `blockx`).
// The function fails if the provided bech32 address is invalid.
func GetBlockxAddressFromBech32(address string) (sdk.AccAddress, error) {
