...
import (
	...
	blockxtypes "github.com/defi-ventures/bcx-testnet-7/types"
...
func (suite *UpgradeTestSuite) TestMigrateClaim() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			suite.SetupTest(blockxtypes.TestnetChainID + "-2") // reset
