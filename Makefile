...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
BLOCKX_BINARY = blockxd
BLOCKX_DIR = blockx
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/defi-ventures/bcx-testnet-5.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=blockx \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(BLOCKX_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm blockx || true
	# create a new container from the latest image
	$(DOCKER) create --name blockx -t -i ${DOCKER_IMAGE}:latest blockx
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp blockx:/usr/bin/blockxd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/defi-ventures/bcx-testnet-7/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(BLOCKX_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(BLOCKX_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(BLOCKX_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(BLOCKX_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/blockxtures/blockx/v1
.PHONY: format
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(BLOCKX_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/blockx:Z blockx/node "./blockxd testnet init-files --v 4 -o /blockx --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\blockxd:/blockx\Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)\build\node1\blockxd:/blockx\Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)\build\node2\blockxd:/blockx\Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)\build\node3\blockxd:/blockx\Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
else
	@docker run --rm -v $(CURDIR)/build/node0/blockxd:/blockx:Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)/build/node1/blockxd:/blockx:Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)/build/node2/blockxd:/blockx:Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
	@docker run --rm -v $(CURDIR)/build/node3/blockxd:/blockx:Z blockx/node "./blockxd tendermint unsafe-reset-all --home=/blockx"
endif
...
PACKAGE_NAME:=github.com/defi-ventures/bcx-testnet-7
